...
function Observer(data_instance) {
  ...
  Object.keys(data_instance).forEach(key =>{
    ...
    Object.defineProperty(data_instance,key,{
      ...
      configurable: true,
      get(){
        // 不能直接返回属性值，因为这样返回的就是undefined了，当使用Object.defineProperty之后，值就会改变
        // console.log(Dependency.temp, 'getter')
        // 当有订阅者发起订阅。发布者就需要更新订阅者名单,因为会多次触发getter函数，需要做过滤
...
function Compile (element, vm) {
  ...
  // 放入临时内存;把当前element下的所有子节点加到fragment中
  const fragment = document.createDocumentFragment()
  let children
  // 每当我执行一次fragment.append(children)，当前这个children就会从vm.$el.children数组中删除，直到最后一个被append，下一次判断条件就会为false，循环结束
  ...
  function fragment_compile(node) {
    ...
    if(node.nodeType === 3) {
      ...
      if(result_regex) {
        ...
        // 拿到当前节点对应的最新的值（兼容深层嵌套），这样就可以把value更新到对应的node
 ...
 */
class Watcher {
  // vm: vue实例，key：实例对应的属性，callback：记录如何更新文本内容
  constructor(vm, key, callback) {
    this.vm = vm;
    this.key = key;
    this.callback = callback
    // 临时属性 - 触发getter,根据作用域的功能，可以最大限度访问这个临时属性
    Dependency.temp = this
    // 为了能够正确的触发对应属性的getter
    key.split('.').reduce((total, current) => total[current], vm.$data)
    // 触发getter以后，需要把临时属性清空
